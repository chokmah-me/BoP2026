...
This is emergent coordination failure. The model produces it without any actor intending it. That is the point.
...
US and Russia both default to force_withdrawal as their top action. This is not surrender; it reflects the AI's tendency to de-escalate when domestic constraint is active (`domestic < 40` blocks high-escalation military actions), then re-escalate once constraint lifts. The cycle is: deploy → domestic cost → withdraw → domestic recovers → deploy. This mirrors the action-reaction cycles in deterrence theory (Jervis 1976; Schelling 1966).
...
**Sweep A, Actor behavior**: varying RU and CN riskTolerance across the 0–1 range. These are the two most consequential actors for escalation. If lowering their risk tolerance produces wins, it confirms that actor-level preferences are the binding constraint. If wins remain at 0%, the structural conditions (scenario design, cascade thresholds) dominate actor behavior.
